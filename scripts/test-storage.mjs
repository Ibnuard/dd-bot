 ...
 *
 * Exercises savePage/list/dedup/clear/concurrent writes against a tmp dir.
 */
...
const { listSaved, getSavedAt, savePage, removeSavedAt, clearSaved } =
...
let r = await savePage(CHAT, 'https://example.com/page1');
assert(r.added === true && r.count === 1, 'first save should add');

r = await savePage(CHAT, 'https://example.com/page1');
assert(r.added === false && r.count === 1, 'duplicate save should be rejected');

r = await savePage(CHAT, 'https://example.com/page2');
...
assert(
  second && second.sourceUrl === 'https://example.com/page2',
...
const concurrent = await Promise.all(
  Array.from({ length: 10 }, (_, i) =>
    savePage(CHAT, `https://example.com/burst-${i}`)
  )
);
assert(
...
const OTHER = 99999;
await savePage(OTHER, 'https://other.example.com/x');
assert((await listSaved(OTHER)).length === 1, 'other chat has 1');
assert((await listSaved(CHAT)).length === 12, 'main chat unchanged');

// removeSavedAt
const rem = await removeSavedAt(CHAT, 0);
assert(rem.removed === true && rem.count === 11, 'remove should report ok');
assert(
  (await getSavedAt(CHAT, 0)).sourceUrl === 'https://example.com/page2',
  'after remove, index 0 should shift'
);
const remOob = await removeSavedAt(CHAT, 99);
assert(remOob.removed === false, 'oob remove returns removed=false');
